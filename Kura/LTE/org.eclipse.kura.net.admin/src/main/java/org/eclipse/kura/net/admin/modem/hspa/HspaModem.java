...
public class HspaModem implements HspaCellularModem {
    ...
    private final int commandTimeout = 2000;
    ...
    public String getRevisionID() throws KuraException {
        synchronized (s_atLock) {
            if (this.m_revisionId == null) {
                ...
                try {
                    reply = commAtConnection.sendCommand(HspaModemAtCommands.getRevision.getCommand().getBytes(), commandTimeout,
                            100);
                } catch (IOException e) {
                    closeSerialPort(commAtConnection);
                    throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
        ...
        synchronized (s_atLock) {
            if (this.m_imsi == null) {
                if (isSimCardReady()) {
                    ...
                    try {
                        reply = commAtConnection.sendCommand(HspaModemAtCommands.getIMSI.getCommand().getBytes(), commandTimeout,
                                100);
                    } catch (IOException e) {
                        closeSerialPort(commAtConnection);
                        throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
        ...
        synchronized (s_atLock) {
            if (this.m_iccid == null) {
                if (isSimCardReady()) {
                    ...
                    try {
                        reply = commAtConnection.sendCommand(HspaModemAtCommands.getICCID.getCommand().getBytes(), commandTimeout,
                                100);
                    } catch (IOException e) {
                        closeSerialPort(commAtConnection);
                        throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
    ...
    protected boolean isAtReachable(CommConnection connection) {
        ...
        do {
            try {
                ...
                byte[] reply = connection.sendCommand(HspaModemAtCommands.at.getCommand().getBytes(), commandTimeout, 100);
                if (reply.length > 0) {
    ...
    public boolean isSimCardReady() throws KuraException {
        ...
        synchronized (s_atLock) {
            ...
            try {
                reply = commAtConnection.sendCommand(HspaModemAtCommands.getSimPinStatus.getCommand().getBytes(), commandTimeout,
                        100);
            } catch (IOException e) {
                closeSerialPort(commAtConnection);
                throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
