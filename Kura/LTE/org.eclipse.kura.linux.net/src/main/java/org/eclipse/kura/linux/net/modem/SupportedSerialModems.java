...
public class SupportedSerialModems {
    ...
    private static void worker() {
        ...
        // SolidRun_Quectel_EC25 serial modem
        if (OS_VERSION != null
                && OS_VERSION.equals(
                        KuraConstants.Raspberry_Pi.getImageName() + "_" + KuraConstants.Raspberry_Pi.getImageVersion())
                && TARGET_NAME != null && TARGET_NAME.equals(KuraConstants.Raspberry_Pi.getTargetName())) {
            modem = SupportedSerialModemInfo.SolidRun_Quectel_EC25;
        ...
        if (modem != null) {
            ...
            try {
                if (!SupportedUsbModems.isAttached(SupportedUsbModemInfo.Telit_HE910_D.getVendorId(),
                    ...
                    if (!SupportedUsbModems.isAttached(SupportedUsbModemInfo.Quectel_EC25.getVendorId(),
                            SupportedUsbModemInfo.Quectel_EC25.getProductId())) {
                        logger.warn("USB modem Quectel_EC25 {}:{} is not detected ...",
                                SupportedUsbModemInfo.Quectel_EC25.getVendorId(),
                                SupportedUsbModemInfo.Quectel_EC25.getProductId());
                        if (modem.getDriver().install() == 0) {
                            for (String modemModel : modem.getModemModels()) {
                                if (modemModel.equals(modem.getDriver().getModemModel())) {
                                    logger.info(
                                            "Driver for the {Quectel_EC25} modem has been installed. Modem is reachable as serial device.",
                                            modemModel);
                                    EventAdmin eventAdmin = serviceTracker.getService();
